...
export const hostRun: McpToolDefinition = {
  tool: {
    name: 'host_run',
    description:
      "Run a container/compose build, service management, or read-only inspection command on the host (outside your container). Restricted to: `podman build`, `docker build`, `podman logs`, `docker logs`, `podman ps`, `docker ps`, `podman stop`, `docker stop` (named containers only — `--all` is refused), `podman compose build|run|up|down|start|stop|restart|ps|logs`, `docker compose build|run|up|down|start|stop|restart|ps|logs`, and read-only tailscale (`status|ip|netcheck|ping|whois|version|licenses|list|metric`, plus `dns|lock|exit-node status|list`), optionally prefixed with a single `cd <abs-path> &&`. Anything else (including `make build`, `./build.sh`, shell-chained commands with `;`/`&&`/`|`, command substitution) is refused. Use case: rootless podman builds that can't run inside the unprivileged container because they need newuidmap, starting/stopping compose services, and reading container logs/status on the host (e.g. `podman logs caddy`). Do not pass `-f`/`--follow` to a logs command — it never exits and will burn the whole timeout. Every request goes through an admin approve/deny on the user's DM channel — there is a human-in-the-loop delay before the command actually runs. Fire-and-forget: returns an ack immediately; the result (or refusal) arrives later as a system chat message. Default cwd /var/home/vmaz/dev; default timeout 600s, max 1800s. For host restarts use the `restart_host` MCP tool instead — never try to systemctl/launchctl/pkill nanoclaw through this tool.",
    inputSchema: {
