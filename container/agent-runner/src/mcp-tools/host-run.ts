...
export const hostRun: McpToolDefinition = {
  tool: {
    name: 'host_run',
    description:
      "Run a container/compose build on the host (outside your container). Restricted to: `podman build`, `docker build`, `podman compose build|run`, `docker compose build|run`, optionally prefixed with a single `cd <abs-path> &&`. Anything else (including `make build`, `./build.sh`, shell-chained commands with `;`/`&&`/`|`, command substitution) is refused. Use case: rootless podman builds that can't run inside the unprivileged container because they need newuidmap. Every request goes through an admin approve/deny on the user's DM channel — there is a human-in-the-loop delay before the command actually runs. Fire-and-forget: returns an ack immediately; the result (or refusal) arrives later as a system chat message. Default cwd /var/home/vmaz/dev; default timeout 600s, max 1800s. For host restarts use the `restart_host` MCP tool instead — never try to systemctl/launchctl/pkill nanoclaw through this tool.",
    inputSchema: {
